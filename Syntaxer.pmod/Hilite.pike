...
protected function         add    = buffer->add;
...
mapping(string:string) get_colors()
{
  return colors;
}

mapping(string:multiset(string)) get_keywords()
{
  return keywords;
}

mapping(string:array(string)) get_styles()
{
  return styles;
}
...
protected string|int get_color_key(string word)
{
  ...
  foreach (keywords; string key; multiset words)
    if ( words[word] )
      return key;
...
{
  string t = reverse(sprintf("%O", this_object()));
  return reverse( t[..search(t, "/")-1] ) - "(" - ")";
}

string _sprintf(int t)
{
  return t == 'O' && sprintf("%s()", object_name());
}
